...
def process_frame(img0, model, stride, names, device, target_class="bat"):
    ...
    if hand_wrist_positions:
        ...
        farthest = max(hand_wrist_positions, key=lambda pt: np.hypot(pt[0] - body_center[0], pt[1] - body_center[1]))
        x_line = farthest[0]
    ...
    for det in pred:  # det는 [N×6] 형태의 tensor (N = 검출된 박스 개수)
        ...
        for row in det:
            ...
            if lines is not None:
                ...
                extended_pt1 = (int(pt1[0] - unit_vec[0] * length), int(pt1[1] - unit_vec[1] * length))
                extended_pt2 = (int(pt2[0] + unit_vec[0] * length), int(pt2[1] + unit_vec[1] * length))
...
def detect_video_vertical(
    video_path, target_class="bat", weights="runs/train/final_learn/weights/best.pt", rotate=False
):
    """
